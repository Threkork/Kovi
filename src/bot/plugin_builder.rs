...
impl PluginBuilder {
    pub fn new(bot: Arc<Mutex<Bot>>, api_tx: mpsc::Sender<Value>) -> Self {
        let (host, port) = {
            let bot_lock = bot.lock().unwrap();
            (
    ...
    pub fn set_info(&mut self, name: &str) {
        ...
        let mut bot_lock = self.bot.lock().unwrap();
    ...
    pub fn build_runtime_bot(&self) -> RuntimeBot {
        RuntimeBot {
            main_admin: self.bot.lock().unwrap().information.main_admin,
            host: self.host,
            port: self.port,
...
impl PluginBuilder {
        ...
        F: Fn(&OnMsgEvent) -> Result<(), ()> + Send + Sync + 'static,
    {
        ...
        for plugin in &mut bot.lock().unwrap().plugins {
